...
from model import RNNAttModel2
...
def main(opt):
    ...
    writer = SummaryWriter(log_dir='logs_model2')
    # opt.is_eval = True
    print('>>> create models')
    in_features = opt.in_features  # 66
    d_model = opt.d_model
    kernel_size = opt.kernel_size
    net_pred = RNNAttModel2.RNNAttModel(in_features=in_features, kernel_size=kernel_size, d_model=d_model,
...
def run_model(net_pred, optimizer=None, is_train=0, data_loader=None, epo=1, opt=None):
    ...
    for i, (p3d_h36) in enumerate(data_loader):
        ...
        p3d_out_all, _ = net_pred(p3d_src, input_n=in_n, output_n=out_n, itera=itera)  #the residual vector of predictions from N-M to N+T frames, the last M+T=20 frames (only dim to use)
