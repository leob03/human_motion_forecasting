...
from model import RNNAttModel1
...
def main(opt):
    ...
    net_pred = RNNAttModel1.RNNAttModel(in_features=in_features, kernel_size=kernel_size, d_model=d_model,
...
def run_model(net_pred, optimizer=None, is_train=0, data_loader=None, epo=1, opt=None):
    ...
    itera = 3
    memory = torch.rand(opt.batch_size, 20, 66, requires_grad=True).float().cuda()
    ...
    for i, (p3d_h36) in enumerate(data_loader):
        ...
        p3d_out_all, memory = net_pred(p3d_src, memory, input_n=in_n, output_n=10, itera=itera)
        #p3d_out_all.shape = (batch_size, seq_in + out_n=M+T, itera=3, 66)

        p3d_out_all = p3d_out_all[:, seq_in:].transpose(1, 2).reshape([batch_size, 10 * itera, -1])[:, :out_n]
        #p3d_out_all[:, seq_in:] only keeps the T last frames for each itera, which is what we want (the predicitons)
        #.transpose(1, 2) -> shape = (batch_size, itera=3, 10, 66)
        #.reshape([batch_size, 10 * itera, -1]) -> shape = (batch_size, 30, 66) concatenates the 3 itera
        #[:, :out_n] -> shape = (batch_size, 10, 66) only keeps the T first frames (the predicitons of the first iteration?)
