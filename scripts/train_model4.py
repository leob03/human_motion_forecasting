...
def main(opt):
    ...
    writer = SummaryWriter(log_dir='logs_model4')
    # opt.is_eval = True
    print('>>> create models')
    in_features = opt.in_features  # 66
    d_model = opt.d_model
    kernel_size = opt.kernel_size
    net_pred = Model4.AttModel(in_features=in_features, kernel_size=kernel_size, d_model=d_model,
                                 num_stage=opt.num_stage, dwt_n=opt.dwt_n)
    net_pred.cuda()
