...
def main(opt):
    ...
    writer = SummaryWriter(log_dir='logs_trbs1_tbs1')
    # opt.is_eval = True
    print('>>> create models')
    ...
    if not opt.is_eval:
        ...
        for epo in range(start_epoch, opt.epoch + 1):
            ...
            writer.add_scalar('Error/testing', ret_test['#10'], epo)
...
def run_model(net_pred, optimizer=None, is_train=0, data_loader=None, epo=1, opt=None):
    ...
    for i, (p3d_h36) in enumerate(data_loader):
        ...
        p3d_out_all, memory = net_pred(p3d_src, memory, input_n=in_n, output_n=out_n, itera=itera)  #the residual vector of predictions from N-M to N+T frames, the last M+T=20 frames (only dim to use)
