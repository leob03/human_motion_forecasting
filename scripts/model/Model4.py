...
class AttModel(Module):
    ...
    def __init__(self, in_features=48, kernel_size=5, d_model=512, num_stage=2, dwt_n=10):
        ...
        self.gcn = GCN.GCN(input_feature=(dwt_n) * 2, hidden_feature=d_model, p_dropout=0.3,
                           num_stage=num_stage,
                           node_n=in_features)
    ...
    def forward(self, src, output_n=25, input_n=50, itera=1):
        ...
        # dwt_m, idwt_m = util.get_dwt_matrix(self.kernel_size + output_n, wavelet)
        # dct_m, idct_m = util.get_dct_matrix(self.kernel_size + output_n)
        # dwt_m = torch.from_numpy(dwt_m).float().cuda()
        #dct_m.shape = [M + T,M + T]
        # idwt_m = torch.from_numpy(idwt_m).float().cuda()

        vn = input_n - self.kernel_size - output_n + 1 #= N - M - T + 1 = 31, number of subsequences
        ...
        # src_value_tmp = torch.matmul(dwt_m[:dwt_n].unsqueeze(dim=0), src_value_tmp).reshape(
        #     [bs, vn, dwt_n, -1]).transpose(2, 3).reshape(
        #     [bs, vn, -1])
        src_value_tmp = util.dwt_transf(src_value_tmp).reshape(
            [bs, vn, dwt_n, -1]).transpose(2, 3).reshape(
            [bs, vn, -1])
        ...
        for i in range(itera):
            ...
            dwt_att_tmp = torch.matmul(att_tmp, src_value_tmp)[:, 0].reshape(
                [bs, -1, dwt_n])
            ...
            # dwt_in_tmp = torch.matmul(dwt_m[:dwt_n].unsqueeze(dim=0), input_gcn).transpose(1, 2) #convert input to dct components
            dwt_in_tmp = util.dwt_transf(input_gcn).transpose(1, 2) #convert input to dct components

            #dct_in_tmp.shape = [32,66,20]
            dwt_in_tmp = torch.cat([dwt_in_tmp, dwt_att_tmp], dim=-1) #concatenate the input and the values weighted by attention
            #dct_in_tmp.shape = [32,66,40]
            dwt_out_tmp = self.gcn(dwt_in_tmp)
            #dct_out_tmp.shape = [32,66,40]
            
            # out_gcn = torch.matmul(idwt_m[:, :dwt_n].unsqueeze(dim=0),
            #                        dwt_out_tmp[:, :, :dwt_n].transpose(1, 2)) #convert back to 3D coordinates (from frequency domain to space domain)
            
            out_gcn = util.dwt_inv_transf(dwt_out_tmp[:, :, :dwt_n].transpose(1, 2))
            ...
            if itera > 1:
                ...
                idx_dwt = np.expand_dims(np.arange(vl), axis=0) + \
                ...
                #idx_dct.shape = [10,20]
                src_key_tmp = src_tmp[:, idx_dwt[0, :-1]].transpose(1, 2)
                ...
                src_dwt_tmp = src_tmp[:, idx_dwt].clone().reshape(
                    [bs * self.kernel_size, vl, -1])

                # src_dwt_tmp = torch.matmul(dwt_m[:dwt_n].unsqueeze(dim=0), src_dwt_tmp).reshape(
                #     [bs, self.kernel_size, dwt_n, -1]).transpose(2, 3).reshape(
                #     [bs, self.kernel_size, -1])
                src_dwt_tmp = util.dwt_transf(src_dwt_tmp).reshape(
                    [bs, self.kernel_size, dwt_n, -1]).transpose(2, 3).reshape(
                    [bs, self.kernel_size, -1])

                src_value_tmp = torch.cat([src_value_tmp, src_dwt_tmp], dim=1)
